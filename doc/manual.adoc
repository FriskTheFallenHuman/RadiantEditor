...
RadiantEditor is a level editor for Idtech 4 Games (Doom 3, Quake 4, Prey), originally developed by The Dark
Mod team.

This User Guide describes the features and functionality of RadiantEditor, however
...
installed into a directory tree which is accessible both by RadiantEditor (for
editing) and the game engine itself (for playing the mission).

Once assets have been arranged in RadiantEditor according to the wishes of the
...
realtime, making use of the same assets that were accessed by RadiantEditor during
editing.

RadiantEditor also maintains its own separate file containing various editing
...
accompanying `.project` file can still be edited with RadiantEditor, although
certain RadiantEditor-specific state may be lost.
...
which acts as the parent for new brushes and patches created in RadiantEditor.
...
When running RadiantEditor for the first time, the *Game Setup* dialog will be
shown. This is where you configure the path to the game installation from which
RadiantEditor will load assets, such as textures, models and entity definitions.

image::GameSetupDialog.png[align="center"]

The *Game Setup* dialog contains the following options:

*Game Type*:: RadiantEditor ships with support for several different game engines,
...
RadiantEditor interface.

NOTE: It is possible to *Cancel* the *Game Setup* dialog and proceed to the main
window without configuring a game installation, in which case RadiantEditor will
show a warning and ask if you wish to proceed. If you do, RadiantEditor will run
but there will be no available textures, models, entities or other game assets.

=== Interface layout

RadiantEditor uses a flexible layout system based on dockable windows which can be moved into
...
RadiantEditor defaults to a black-on-white colour scheme in the 2D windows, but ships with
...
Colour schemes only affect what is rendered in the 2D and 3D views by RadiantEditor itself.
...
IMPORTANT: The fully lit rendering mode in RadiantEditor is not identical to what
...
TIP: Like other editors in the Radiant family, RadiantEditor offers a rather
...
RadiantEditor provides six buttons to quickly flip or rotate objects (in 90 degree
...
the system used by RadiantEditor may seem somewhat back-to-front. In previous
games, the world starts out as an infinite solid, in which you "carve out" rooms
using subtractive brushes. In RadiantEditor, the world starts out as an infinite
...
that way. RadiantEditor offers several options for creating multi-sided brushes in
...
RadiantEditor provides a couple of shortcuts. These options can be found on the
...
is useful, so it is retained in RadiantEditor.
|===
...
. Select a brush.
. Activate *Select Edges* with the toolbar button or *E* key. RadiantEditor will
...
Just like with brushes, RadiantEditor offers several default patch shapes beyond
...
3 and RadiantEditor does not permit the creation of a perfect sphere).
...
When RadiantEditor first starts up no textures are loaded and this panel is empty.
...
face. RadiantEditor provides the *Surface Inspector* for this purpose, which can
...
order to assist with this, RadiantEditor provides a *shader clipboard* which
...
RadiantEditor provides certain common functionality to all entities, such as the
...
and patches created inside RadiantEditor, or from an external model file in ASE or
...
view and choose *Create model…*. RadiantEditor will show the model selector
dialog:
...
2D view and choose *Create player start here*. RadiantEditor will create the
player start entity at the clicked position.

Since it makes no sense to have more than one player start location, RadiantEditor
...
When you create a model using the <<CreatingModels,model selector>>, RadiantEditor
...
primitives created within RadiantEditor. In either case, the behaviour of
...
continuously. RadiantEditor offers dedicated commands to convert to/from a
...
*TAB* will cause RadiantEditor to select a different primitive contained within
...
RadiantEditor does not include functionality for compiling a map into the form
...
and RadiantEditor to be visible on the screen as separate windows, which you can
...
After the map compilation has failed with a leak, return to RadiantEditor and
...
Since a fully developed Dark Mod map is large and complex, RadiantEditor provides
...
RadiantEditor allows a number of objects to be grouped together, so that they can
...
RadiantEditor allows a collection of objects to be saved to disk as a *prefab*,
...
prefabs are a RadiantEditor-only feature that have no significance to the engine.
...
RadiantEditor provides a limited ability to export scene geometry into a number of
...
frequently-used map elements — RadiantEditor is not intended to compete with
...
. Select the geometry to be exported. Note that RadiantEditor currently does not
...
objects, RadiantEditor also provides a mechanism for controlling the visibility of
...
between RadiantEditor sessions.
...
In addition to the filters supplied with the mod distribution, RadiantEditor also
...
become more challenging. To assist with this, RadiantEditor provides an *Entity
...
configure manually by entering property values on entities. RadiantEditor
...
contact the distributor of your RadiantEditor binary package for advice, or
examine the build options if you are compiling RadiantEditor yourself from source
code.
...
RadiantEditor provides a Difficulty editor dialog to facilitate one particular
...
current map has customised difficulty names RadiantEditor will adjust the tab
names accordingly.
...
*Editing the difficulty name*:: To edit the name of the current difficulty
 ...
 `worldspawn` entity and reflected both in game and in RadiantEditor.
...
The Dark Mod includes functionality to dynamically interact with a RadiantEditor
...
These features are accessed in RadiantEditor through the *Connection* menu and
buttons on the camera view toolbar.

==== Activating the connection

. In *The Dark Mod*, load the map which you are currently editing in RadiantEditor.
...
The game process is now ready to exchange data with RadiantEditor.

==== Synchronising camera position

You can synchronise the RadiantEditor camera position and the game player position
...
|Game position follows RadiantEditor camera|
Any motion of the RadiantEditor camera will be transmitted in realtime to the
...
Update the RadiantEditor camera to match the current player position and view
direction in game.

This is a single-shot command; there is no mechanism to continuously move the
RadiantEditor camera in response to player motion in game.
|===
...
*Save selected as Collision Model...*:: Allows a selected map object (typically
 ...
 After choosing this option, RadiantEditor will display the model chooser dialog,
...
*Reload Models/Selected Models/Skins/Scripts/Readable Guis/Materials/Defs/Particles*::
 Forces RadiantEditor to re-read the selected category of items from disk,
 refreshing its internal data structures. This is necessary if you have edited
 or re-exported some asset which RadiantEditor is using in the current map and you
...
*Exit*:: Exit RadiantEditor, prompting to save the current map if necessary.
...
== Configuring RadiantEditor

RadiantEditor offers a large number of configurable options which can be used to
...
*Higher selection priority for entities*:: When using the mouse to click on and
 ...
 controls whether RadiantEditor will prefer to select the entity (enabled) or the
 brush (disabled).

==== Multi Monitor settings

The Multi Monitor page contains a single setting, *Start RadiantEditor on
monitor*, which allows you to choose which monitor RadiantEditor will start on if
...
*Autosave Interval*:: Use this slider to choose how often RadiantEditor will make
 an autosave. By default, autosaves are made every 5 minutes.

*Save Snapshots*:: If this option is enabled, RadiantEditor will make each
 ...
 against RadiantEditor or system crashes but not against long-term erroneous
 changes.
...
*Max Snapshot size per map*:: This allows you to control the amount of disk
 ...
 grows above this limit, RadiantEditor will start to delete old snapshots.
...
*Ignore light volume bounds when calculating default rotation pivot location*::
 ...
 when multiple objects are selected. RadiantEditor will place the rotation widget
...
*Undo Queue Size*:: Use this spinbox to control the number of operations which
 are saved onto RadiantEditor's internal undo stack. Larger values provide a
...
*Open last map on startup*:: Enable this option to make RadiantEditor
...
RadiantEditor is designed to work out of the box with a standard Doom 3 or Dark
...
RadiantEditor offers a mechanism to deal with this by allowing certain assets to
...
RadiantEditor. This means that in order to hide specific shaders, you must move
