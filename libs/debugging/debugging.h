...
#define ASSERT_MESSAGE(condition, message)\
	if(!(condition)) { GlobalErrorHandler()("RadiantEditor - Assertion Failure", std::string(FILE_LINE) + "\nAssertion failure: " + message + "\nBreak into the debugger?"); }

/// \brief Sends a \p message to the current debug-message-handler text-output-stream.
#define ERROR_MESSAGE(message)\
{ GlobalErrorHandler()("RadiantEditor - Runtime Error", std::string(FILE_LINE) + "\nRuntime Error: " + message + "\nBreak into the debugger?"); }
