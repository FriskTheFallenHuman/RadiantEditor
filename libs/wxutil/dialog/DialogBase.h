...
/**
 * \brief Base class for many RadiantEditor dialogs.
 *
...
public:
    /// Construct and initialise a dialog that is a child of RadiantEditor's main window (if present)
    ...
    // dialog will try to be a child to RadiantEditor's main window)
    DialogBase(const std::string& title, wxWindow* parent);

    // Construct and initialise a named dialog that is a child of RadiantEditor's main window (if present)
    ...
    /**
     * Adjust this window to fit the display RadiantEditor is currently (mainly)
